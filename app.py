...
def venues():
  ...
  data_ = []
  s = set(map(lambda d : d.city+","+d.state , venues))
  ...
  for d in data_:
    for v in venues:
        if d["state"] == v.state and d["city"] == v.city:
            d["venues"].append({
                "id": v.id,
                "name": v.name,
                "num_upcoming_shows": db.session.query(Show).filter(Show.venue_id==v.id).count()})
...
def show_venue(venue_id):
  ...
  venue_query = db.session.query(Venue).get(venue_id) # get option only for primary_key; can use filter
...
def search_artists():
  ...
  for artist in search_result:
    ...
    {"id": artist.id,
    "name": artist.name,
    "num_upcoming_shows": db.session().query(Show).filter(Show.artist_id == artist.id).count() }
    )
...
def show_artist(artist_id):
  ...
  upcoming_shows_query = db.session.query(Show).join(Venue).filter(Artist.id==artist_id).filter(Show.start_time>=datetime.now())
  upcoming_shows = []
  ...
  past_shows_query = db.session.query(Show).join(Venue).filter(Artist.id==artist_id).filter(Show.start_time<datetime.now())
  past_shows = []
...
@app.route('/shows')
def shows():
  show_data = db.session.query(Show).join(Artist).join(Venue).all()
  shows_data=[]
